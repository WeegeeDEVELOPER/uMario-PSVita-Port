...
void LoadingMenu::Draw(SDL_Renderer* rR) {
	...
	if(loadingType) {
		...
		CCFG::getText()->Draw(rR, "WORLD", 320 + psvitaOffset, 144);
		CCFG::getText()->Draw(rR, CCore::getMap()->getLevelName(), 416 + psvitaOffset, 144);

		CCore::getMap()->getPlayer()->getMarioSprite()->getTexture()->Draw(rR, 342 + psvitaOffset, 210 - CCore::getMap()->getPlayer()->getHitBoxY()/2);

		CCFG::getText()->Draw(rR, "y", 384 + psvitaOffset, 208);

		if(CCore::getMap()->getPlayer()->getNumOfLives() > 9) {
			CCore::getMap()->getBlock(180)->getSprite()->getTexture()->Draw(rR, 410 + psvitaOffset, 210);
		}
		CCFG::getText()->Draw(rR, std::to_string(CCore::getMap()->getPlayer()->getNumOfLives()), 432 + psvitaOffset, 208);
		//CCFG::getText()->DrawCenterX(rR, "REMEMBER THAT YOU CAN RUN WITH " + CCFG::getKeyString(CCFG::keyIDShift), 400, 16);
	} else {
		CCore::getMap()->DrawGameLayout(rR);
		CCFG::getText()->DrawCenterX(rR, "GAME OVER", 240 + psvitaOffset, 16);
	}
}
